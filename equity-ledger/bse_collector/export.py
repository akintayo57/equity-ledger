...
from bse_collector.db import get_db_connection, get_exchange_id
from bse_collector.config import DEFAULT_DB_PATH, DEFAULT_EXPORT_DIR

logger = logging.getLogger(__name__)

def export_dataset(db_path: Path = DEFAULT_DB_PATH, output_dir: Path = DEFAULT_EXPORT_DIR) -> dict[str, int]:
    """
    Exports BSE database records as CSV tables, including a consolidated historical CSV file.
    Returns:
    ...
    # 1. Export bse_securities.csv
    securities_path = output_dir / "bse_securities.csv"
    cursor.execute("""
    ...
    counts["bse_securities.csv"] = len(securities_rows)
    logger.info(f"Exported {len(securities_rows)} BSE securities to {securities_path}")

    # 2. Export bse_sessions.csv
    sessions_path = output_dir / "bse_sessions.csv"
    cursor.execute("""
    ...
    counts["bse_sessions.csv"] = len(sessions_rows)
    logger.info(f"Exported {len(sessions_rows)} BSE trade sessions to {sessions_path}")

    # 3. Export bse_prices.csv
    prices_path = output_dir / "bse_prices.csv"
    cursor.execute("""
    ...
    counts["bse_prices.csv"] = len(prices_rows)
    logger.info(f"Exported {len(prices_rows)} BSE prices to {prices_path}")

    # 4. Export consolidated Kaggle CSV (bse_historical_prices.csv)
    kaggle_path = output_dir / "bse_historical_prices.csv"
    cursor.execute("""
        SELECT 
    ...
    counts["bse_historical_prices.csv"] = len(processed_kaggle_rows)
    logger.info(f"Exported {len(processed_kaggle_rows)} BSE consolidated historical rows to {kaggle_path}")

    # 5. Export bse_dataset_metadata.json
    metadata_path = output_dir / "bse_dataset_metadata.json"
    ...
    metadata = {
        "dataset_name": "Barbados Stock Exchange (BSE) Historical Stock Prices",
        "description": "Historical daily/weekly trade session stock prices on the Barbados Stock Exchange.",
        "source": "Barbados Stock Exchange (https://bse.com.bb/)",
        ...
        "statistics": {
            "total_securities": counts["bse_securities.csv"],
            "total_sessions": counts["bse_sessions.csv"],
            "total_price_records": counts["bse_prices.csv"]
        },
        "files": {
            "bse_securities.csv": {
                "description": "List of companies listed on the BSE exchange.",
    ...
    logger.info(f"Exported BSE dataset metadata to {metadata_path}")
