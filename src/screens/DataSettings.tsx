...
import { PlusCircle, Upload, Trash2, Edit2, Info, User, LogOut } from 'lucide-react';
  ...
  return (
    <div className="space-y-4 pb-6">
      <div className="relative mb-4">
        <label htmlFor="settings-menu-select" className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Settings Menu</label>
        <select
                  ...
                  return (
                    <div key={sec.id} className="p-3 flex justify-between items-center text-sm">
                      <div>
                        ...
                        <div className="text-xs text-slate-500">{sec.exchangeId}</div>
                      </div>
                      <div className="flex items-center space-x-2">
                        ...
                        <button onClick={() => setEditingSecId(sec.id)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded cursor-pointer">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button 
                          onClick={() => handleDeletePrompt(sec.id)} 
                          disabled={!hasData}
                          className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded disabled:opacity-30 disabled:hover:bg-transparent cursor-pointer"
                        >
      ...
      <CardContent className="p-4">
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs text-slate-500 mb-1">EPS ({currency})</label>
      ...
      <CardContent className="p-6 text-center space-y-4">
        ...
        <div className="pt-4 border-t border-slate-100 flex flex-col items-center">
          ...
          <button 
            ...
            className="flex items-center space-x-2 py-2 px-6 border border-rose-200 text-rose-600 rounded-xl hover:bg-rose-50 font-semibold text-sm transition-all cursor-pointer"
          >
