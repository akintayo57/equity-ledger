  ...
  return (
    <div className="space-y-4 pb-6">
      <div className="relative mb-4">
        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Settings Menu</label>
        <select
          value={activeSection}
          onChange={(e) => setActiveSection(e.target.value as any)}
          className="w-full text-sm font-medium border border-slate-300 rounded-lg p-2.5 bg-white shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none appearance-none"
        >
  ...
  return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-2 p-1 bg-slate-200 rounded-lg">
          <button 
            className={`py-2 text-sm font-medium rounded-md transition-colors ${subTab === 'PRICES' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'}`}
            onClick={() => setSubTab('PRICES')}
          >
            Prices
          </button>
          <button 
            className={`py-2 text-sm font-medium rounded-md transition-colors ${subTab === 'FX' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600'}`}
        ...
        <Card className="mt-8 opacity-75">
          <CardHeader title="System" />
          <CardContent className="p-4 flex flex-col space-y-2">
             <button className="w-full py-2 bg-slate-100 text-slate-600 rounded text-sm text-center">Export Prices (CSV)</button>
          </CardContent>
        </Card>
      </div>
  );
...
const FundamentalsSection = () => {
  const { securities, updateSecurity } = useStore();
          ...
          <Card>
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <textarea 
                ...
                className="w-full text-xs font-mono border border-slate-300 rounded p-2 h-24 focus:ring-blue-500 focus:border-blue-500 bg-white"
              />
              <button 
                onClick={handleImport} 
                disabled={!csvText.trim()}
                className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-semibold flex justify-center items-center disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                ...
                {securities.map(sec => {
                  ...
                  return (
                    <div key={sec.id} className="p-3 flex justify-between items-center text-sm">
                      <div>
                        <div className="font-bold text-slate-900">{sec.ticker}</div>
                        <div className="text-xs text-slate-500">{sec.exchange}</div>
                      </div>
                      <div className="flex items-center space-x-2">
                        {hasData ? (
                          <span className="text-[10px] uppercase font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded mr-2">Data Present</span>
                        ) : (
                          <span className="text-[10px] uppercase font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded mr-2">No Data</span>
                        )}
                        <button onClick={() => setEditingSecId(sec.id)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded">
...
const EditFundamentalsForm = ({ security, onClose }: { security: Security, onClose: () => void }) => {
      ...
      <CardContent className="p-4">
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-slate-500 mb-1">P/E Ratio</label>
              <input type="number" step="0.01" value={peRatio} onChange={e => setPeRatio(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">EPS ({security.currency})</label>
              <input type="number" step="0.01" value={eps} onChange={e => setEps(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Div. Yield (%)</label>
              <input type="number" step="0.01" value={dividendYield} onChange={e => setDividendYield(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">P/B Ratio</label>
              <input type="number" step="0.01" value={pbRatio} onChange={e => setPbRatio(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">ROE (%)</label>
              <input type="number" step="0.01" value={roe} onChange={e => setRoe(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">As of Date</label>
              <input type="date" value={lastUpdated} onChange={e => setLastUpdated(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
          </div>
          <div className="flex justify-end space-x-2 pt-2 border-t border-slate-200 mt-2">
            <button type="button" onClick={onClose} className="px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-100 rounded">Cancel</button>
            <button type="submit" className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded hover:bg-blue-700">Save</button>
          </div>
        </form>
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent className="p-4 space-y-4">
        <div className="flex items-center space-x-3 mb-4">
          ...
          <div>
            <h3 className="font-bold text-slate-900">Harbour Finance</h3>
            <p className="text-xs text-slate-500">Version 1.0.0-prototype</p>
          </div>
        </div>
...
const PriceUpdatesTab = () => {
  const { securities, addPriceUpdate } = useStore();
    ...
    <>
      <Card>
        ...
        <CardContent className="p-4">
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-xs text-slate-500 mb-1">Security</label>
              <select value={secId} onChange={e => setSecId(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" step="0.01" required value={price} onChange={e => setPrice(Number(e.target.value))} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white" />
              </div>
              <div>
                 <label className="block text-xs text-slate-500 mb-1">Date</label>
                 <input type="date" required value={date} onChange={e => setDate(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white" />
              </div>
            </div>
            <button type="submit" className="w-full mt-2 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-semibold flex justify-center items-center transition-colors">
        ...
        <CardContent className="p-4">
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <select value={toCurrency} onChange={e => setToCurrency(e.target.value as Currency)} className="w-full text-sm border border-slate-300 rounded p-1.5 bg-white focus:ring-2 focus:ring-blue-500 focus:outline-none">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" step="0.001" required value={rate} onChange={e => setRate(Number(e.target.value))} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white" />
              </div>
              <div>
                 <label className="block text-xs text-slate-500 mb-1">Date</label>
                 <input type="date" required value={date} onChange={e => setDate(e.target.value)} className="w-full text-sm border border-slate-300 rounded p-1.5 focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white" />
